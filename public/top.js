...
async function loadTopGames() {
  ...
  try {
    ...
    showAlert(
      "Erro ao carregar jogos. Verifique se o servidor está rodando e se há dados no banco.",
      "danger"
    );
    showEmptyState();
  } finally {
...
function displayGames(games, startIndex = 0) {
  ...
  games.forEach((game, index) => {
    ...
    // Determina a classe do badge usando Bootstrap (Padronização)
    let badgeClass = "bg-secondary";
    const desc = (game.review_score_desc || "").toLowerCase();

    if (
      desc.includes("overwhelmingly positive") ||
      desc.includes("extremamente positivas")
    ) {
      badgeClass = "bg-success";
    } else if (
      desc.includes("very positive") ||
      desc.includes("muito positivas")
    ) {
      badgeClass = "bg-success";
    } else if (desc.includes("positive") || desc.includes("positivas")) {
      badgeClass = "bg-primary";
    } else if (desc.includes("mixed") || desc.includes("neutras")) {
      badgeClass = "bg-warning text-dark";
    } else if (desc.includes("negative") || desc.includes("negativas")) {
                ...
                <div class="card-body p-3">
                    ...
                    ${
                      game.short_description
                        ? `
                        <p class="game-description">${escapeHtml(
                          game.short_description
                        )}</p>
                    `
                        : ""
                    ...
                    ${
                      game.developers
                        ...
                        <p class="text-muted small mb-2">
                            <i class="bi bi-code-square"></i> ${escapeHtml(
                              game.developers
                            )}
                        </p>
                    `
...
async function triggerPreload() {
  ...
  try {
    ...
    if (data.success) {
      showAlert(
        `Pré-carregamento iniciado: ${data.total} jogos em processamento. Aguarde alguns minutos e clique em "Atualizar Lista".`,
        "success"
      );
    } else {
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML =
        '<i class="bi bi-download"></i> Iniciar Pré-carregamento de 100 Jogos';
    }
  }
}
...
async function showComments(appId, gameName) {
  ...
  document.getElementById(
    "modalGameTitle"
  ).textContent = `Comentários - ${gameName}`;
